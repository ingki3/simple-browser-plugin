  ...
  private async maybeShowPdfGuidance(): Promise<void> {
    ...
    this.send({
      ...
      message: [
        ...
        "• 로컬 파일(file:// URL)은 지원되지 않습니다. Chrome 서비스 워커가 file:// 리소스를 가져올 수 없도록 막혀 있기 때문입니다. (확장 설정의 '파일 URL 액세스' 옵션도 이 제약을 풀지 못합니다.)",
        "  대안: PDF를 http(s)로 호스팅되는 URL에서 열거나, 텍스트를 채팅창에 직접 붙여넣어 주세요.",
